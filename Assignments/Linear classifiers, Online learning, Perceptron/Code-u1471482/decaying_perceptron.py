...
perceptron_type = "Decaying Perceptron"
...
def cv_setup(train_fold_df, test_fold_df, learning_rate, weights, bias, epochs):
    for epoch in range(epochs):
        ...
        weights, bias, _ = perceptron(df=train_fold_df, learning_rate=new_learning_rate, weights=weights, bias=bias)
...
def online_setup(train_df, dev_df, learning_rate, weights, bias, epochs):
    ...
    for epoch in range(epochs):
        ...
        weights, bias, update_count = perceptron(
            df=train_df, learning_rate=new_learning_rate, weights=weights, bias=bias
        )
    ...
    log.debug(f"   Best Dev Accuracy: {best_accuracy} (Epoch {best_epoch + 1:>2})")
    print(f"   Best Dev Accuracy: {best_accuracy} (Epoch {best_epoch + 1:>2})")
    print(f"   Average Dev Accuracy: {np.mean(dev_accuracies)}")
