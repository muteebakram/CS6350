...
def calculate_info_gain_T(df):
    ...
    for attr, gain in info_gain_t.items():
        print(f"Attribute: {attr:<10} GainT:  {gain:<25}")
...
def calculate_info_gain_S(df):
    ...
    for attr, gain in info_gain_s.items():
        print(f"Attribute: {attr:<10} GainS:  {gain:<25}")
...
if __name__ == "__main__":
    ...
    # print(get_info_gain(df, "shape", list(df["shape"].unique())))
    print("GainT (S, A)")
    ...
    print(f"\nBest GainT feature for root of the decision tree is '{get_max_dict_value(info_gain_t)}'")
